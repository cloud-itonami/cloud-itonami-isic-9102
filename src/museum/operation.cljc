(ns museum.operation
  "OperationActor -- one museum/cultural-heritage operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (CuratorOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Collections Governor (:govern)
  ...
  One graph run = one museum/cultural-heritage operation (intake ->
  ...
  decision to a human operator (the curator/collections committee). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:item/loan`/`:item/deaccession` ALWAYS reach this node
  when the governor is clean -- see `museum.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [museum.curatoropsllm :as curatoropsllm]
...
(defn build
  ...
  `museum.store/Store`).
  opts:
    :advisor      -- a `museum.curatoropsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (curatoropsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CuratorOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (curatoropsllm/-advise advisor store request)]
            {:proposal p :audit [(curatoropsllm/trace request p)]})))

      ;; Collections Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
